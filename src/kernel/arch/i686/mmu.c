...
{
    return higher_half_yet? (paddr_t)(vaddr-HIGHER_HALF_OFFSET) : vaddr;
}    

static inline vaddr_t mmu_ptov(paddr_t paddr)
{
    return higher_half_yet? (vaddr_t)(paddr+HIGHER_HALF_OFFSET) : paddr;
}    
...
{
    vaddr_t va;
    if (flags & MMU_MAP_ID_ENABLE) va = (vaddr_t)addr;
    else va = mmu_ptov(addr);

    usize huge_page_size;
    if (flags & MMU_HUGE_PAGE) huge_page_size=HUGE_PAGE_SIZE();

    for (usize i=0;i<n;i++)
    {
        if (flags & MMU_HUGE_PAGE)
