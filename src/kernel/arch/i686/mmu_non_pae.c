...
void mmu_mmap_non_pae(vaddr_t vaddr, paddr_t paddr, u32 attributes)
{
    ...
    else
    {
        ...
        pd_entry = (u32)page_table & PT_ADDR_MASK;
...
void mmu_mmap_huge_non_pae(vaddr_t vaddr, paddr_t paddr, u32 attributes)
{
    ...
    huge_page_entry |= (MMU_PA_FLAG_PRESENT | MMU_PA_FLAG_PSE);
...
usize mmu_munmap_non_pae(vaddr_t vaddr, paddr_t* paddr_out)
{
    ...
    if (!(pt_entry & MMU_PA_FLAG_PRESENT))
    {
        *paddr_out = NULL;
        return 0;
    }
    ...
    if (pt_entry & MMU_PA_FLAG_PSE)
    {
        pt_entry &= ~MMU_PA_FLAG_PRESENT;
    ...
    if (!(page_entry & MMU_PA_FLAG_PRESENT))
    {
        *paddr_out = NULL;
        return 0;
    }

    page_entry &= ~MMU_PA_FLAG_PRESENT;
...
void mmu_non_pae_init(u32* page_dir_addr)
{
    // As features would have been enabled in the mmu_init() call,
    // We will only focus on assigning page directory

    mmu_non_pae_data.page_directory_phys = mmu_vtop((vaddr_t)page_dir_addr); // Pass vtop() here, in case of 
                                                                    // just in case this function is somehow moved 
                                                                    // after the higher-half transition so that nothing breaks afterwards.

    mmu_non_pae_data.page_directory = (u32*)(mmu_ptov(page_dir_addr)); // Just pass ptov() here, for the same reason above
}

void mmu_non_pae_page_dir_recalibrate()
{
    // As after the higher half transition, the old page directory address may point to nowhere
    // (due to it being ID-mapped before), we will have to recalibrate the address, by running ptov().
    mmu_non_pae_data.page_directory = (u32*)(mmu_ptov(mmu_non_pae_data.page_directory_phys));
}