...
void kernel_prepare_mmu(boot_info_t* info)
{
    ...
    page_alloc_init(bmp_buffer);
    
    // Lock pages where the kernel resides
    ...
    // Then, init page table manager.

    paging_addr = page_alloc_request();
    memset(paging_addr, 0, 0x1000);
    mmu_init_pages(paging_addr, use_pae);
    mmu_exception_init();
    
    // Map the whole kernel (including the bss)
    ...
    // Map the rest of the total memory as RO initially
    usize mem_size_bytes = mmu_get_memory_size();
    usize mem_page_count = mmu_byte_to_page_count(mem_size_bytes-(usize)paging_addr+1);
    mmu_mmapn((usize)paging_addr, 0, mem_page_count, MMU_PT_FLAG_READ_WRITE);
    
    // Remap anything that's kernel related

    // NOTE: Please don't touch on anything
    // Scramble these up and then you'll be sitting for days wondering why
    // the kernel page faults (Maybe a bug, as if the font glyph is mapped RO first,
    // then all the components that follow will be RO, and won't care about "RW"?)
    // Weird.
