...
)
{
    ...
    mp_table_drv_node = driver_add_to_tree(
        &driver_forest,
        cpu_self,
...
)
{
    ...
    usize page_count = mmu_byte_to_page_count(mpct->length);

    mmu_munmapn((usize)mpct, page_count);

    if (mpct->spec_revision == 4)
    {
        usize ext_page_count = mmu_byte_to_page_count(mpct->ext_fields.ext_length);
...
static mpct_hdr_t* cpu_mpct_check(u32 mpct_phys_base)
{
    ...
    usize page_count = mmu_byte_to_page_count(length);
    ...
    if (valid_mpct->spec_revision == 4 && valid_mpct->ext_fields.ext_length > 0)
    {
        ...
        {
            mmu_munmapn(mpct_phys_base, page_count + ext_page_count);
            return NULL;
        }    
    }
