...
static void page_alloc_reserve(void* address);
static void page_alloc_reserven(void* address, usize page_count);
...
void page_alloc_init(void* bitmap_base)
{
    ...
    page_alloc_data.last_free_idx = 0;
    ...
    usize lo_mem_num_pages = mmu_byte_to_page_count(0x100000);
    page_alloc_reserven((void*)0, lo_mem_num_pages);
...
void* page_alloc_request()
{
    ...
    for (usize i=0;i<bitmap_bit_cnt;i++)
    {
...
static void page_alloc_reserve(void* address)
{
    ...
    page_alloc_data.free_mem -= 4096;
    page_alloc_data.reserved_mem += 4096;
    // Spin-unlock here
}
...
static void page_alloc_reserven(void* address, usize page_count)
{
    ...
    {
        // Spinlock here
        page_alloc_reserve(address+(i<<12));
        // Spin-unlock here
    }
}